...
class Summary extends H5P.EventDispatcher {
  ...
  constructor(config, parent, chapters) {
    ...
    parent.on("bookCompleted", (event) =>
      this.setBookComplete(event.data.completed)
    );
  ...
  setBookComplete(complete) {
    let summaryFooter = this.parent.mainWrapper
      ? this.parent.mainWrapper[0].querySelector(
          ".h5p-interactive-book-summary-footer"
        )
      : null;
    ...
    Array.from(
      document.querySelectorAll(".h5p-interactive-book-summary-menu-button")
    ).forEach((button) =>
      button.setAttribute("data-book-completed", complete.toString())
    );
  }
   ...
   */
  setFilter(filter) {
    const overviewList = this.wrapper.querySelector(
      ".h5p-interactive-book-summary-overview-list"
    );
    const sectionList = Array.from(
      overviewList.querySelectorAll(
        ".h5p-interactive-book-summary-overview-section"
      )
    );
    ...
    const emptyContainer = overviewList.querySelector(
      ".h5p-interactive-book-summary-overview-list-empty"
    );
    emptyContainer.style.display = "none";
    if (filter === this.filterActionUnanswered) {
      overviewList.classList.add(
        "h5p-interactive-book-summary-overview-list-only-unanswered"
      );
      const filteredSectionList = sectionList.filter(
        (section) =>
          !section.classList.contains(
            "h5p-interactive-book-summary-no-interactions"
          )
      );
      if (filteredSectionList.length) {
        filteredSectionList[0].classList.add(
          "h5p-interactive-book-summary-top-section"
        );
  ...
  createSummaryButton() {
    ...
    button.onclick = () => {
      ...
      //Trigger XAPI on "Check Your Progress" button selected
  ...
  createCircle(progress) {
    ...
    circleProgress.classList.add(
      "h5p-interactive-book-summary-progress-circle"
    );
  ...
  createProgress(
    ...
    progressBigText.classList.add(
      "h5p-interactive-book-summary-progressbox-bigtext"
    );
    ...
    progressSmallText.classList.add(
      "h5p-interactive-book-summary-progressbox-smalltext"
    );
    progressSmallText.innerHTML = smallText
      .replace("@count", smallProgress)
      .replace("@total", smallProgressTotal);
  ...
  addScoreProgress() {
    ...
    const circle = box.querySelector(
      ".h5p-interactive-book-summary-progress-circle"
    );
  ...
  addActionButtons() {
    ...
    if(this.behaviour.displayRestartButton) {
   ...
   */
  createRestartButton() {
    const restartButton = this.addButton(
      "icon-restart",
      this.l10n.restartLabel
    );
  ...
  createSectionList(sections, chapterId) {
    ...
    for (const section of sections) {
      ...
      sectionRow.classList.add(
        "h5p-interactive-book-summary-overview-section-details"
      );
      if (this.behaviour.progressIndicators) {
        const icon = document.createElement("span");
        icon.classList.add("h5p-interactive-book-summary-section-icon");
        icon.classList.add(
          section.taskDone ? "icon-chapter-done" : "icon-chapter-blank"
        );
      ...
      const contentDataTitle =
        section.instance.contentData &&
      ...
      title.innerHTML = contentDataTitle
        ? contentDataTitle
        : metadataTitle
        ? metadataTitle
        : "Untitled";
      ...
      if (typeof section.instance.getScore === "function") {
        score.innerHTML = this.l10n.scoreText
          .replace("@score", section.instance.getScore())
          .replace("@maxscore", section.instance.getMaxScore());
      }

      if (section.taskDone) {
        sectionRow.classList.add(
          "h5p-interactive-book-summary-overview-section-details-task-done"
        );
      } else {
    ...
    if (sectionElements.length) {
      ...
      sectionRow.classList.add(
        "h5p-interactive-book-summary-overview-section-score-header"
      );
  ...
  createChapterOverview(chapter) {
    ...
    let { sectionElements: sections, hasUnansweredInteractions } =
      this.createSectionList(
        chapter.sections.filter((section) => section.isTask),
    ...
    sectionSubheader.classList.add(
      "h5p-interactive-book-summary-chapter-subheader"
    );
    if (chapter.maxTasks) {
      sectionSubheader.innerHTML = this.l10n.leftOutOfTotalCompleted
        .replace("@left", Math.max(chapter.maxTasks - chapter.tasksLeft, 0))
        .replace("@max", chapter.maxTasks);
    } else {
  ...
  createFilterDropdown() {
    ...
    const allInteractions = createElement(
      this.l10n.allInteractions,
      this.filterActionAll
    );
    const unansweredInteractions = createElement(
      this.l10n.unansweredInteractions,
      this.filterActionUnanswered
    );
  ...
  addSummaryOverview() {
    ...
    emptySummaryList.classList.add(
      "h5p-interactive-book-summary-overview-list-empty"
    );
  ...
  noChapterInteractions() {
    ...
    wrapper.classList.add(
      "h5p-interactive-book-summary-no-chapter-interactions"
    );
    ...
    normalText.classList.add(
      "h5p-interactive-book-summary-no-initialized-chapters"
    );
  ...
  addSummaryPage(container) {
    ...
    Array.from(
      document.querySelectorAll(".h5p-interactive-book-summary-footer")
